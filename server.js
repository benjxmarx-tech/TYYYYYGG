...
wss.on('connection', (ws) => {
  ...
  ws.on('message', raw => {
    ...
    if (m.type === 'join_room') {
      ...
      if (room.started) {
        // reconecta quem já estava / entra atrasado e recebe START
        client.room = room.id;
        ...
        if (!room.clients.has(client.id)) {
          if (room.clients.size >= 3) {
            send(ws, { type: 'error', message: 'Esse servidor está cheio.' });
            return;
          }
          client.joinOrder = ++room.seq;
          room.clients.set(client.id, client);
        }
        send(ws, { type: 'joined', room: room.id, selfChar: client.charId, ...state(room) });
        send(ws, { type: 'start', players: state(room).players, leaderId: state(room).leaderId });
        sendState(room);
        return;
      }
      if (room.clients.size >= 3 || room.locked) {
        send(ws, {
          type: 'error',
    ...
    if (m.type === 'start_game') {
      ...
      setTimeout(() => broadcast(room, payload), 400);
      setTimeout(() => broadcast(room, payload), 1200);
    ...
    if (m.type === 'enemy_hit' && client.room) {
      const room = rooms.get(client.room);
      if (!room || !room.started) return;
      broadcast(room, {
        type: 'enemy_hit',
    ...
    if (m.type === 'ability_event' && client.room) {
      const room = rooms.get(client.room);
      if (!room || !room.started) return;
      broadcast(room, {
        type: 'ability_event',
        ...
        x: Number(m.x)||0,
        y: Number(m.y)||0,
        radius: Number(m.radius)||70,
        life: Number(m.life)||300
      });
      return;
    }
